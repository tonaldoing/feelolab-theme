...
4. `.github/workflows/release.yml` detecta que esa versión no tiene release en `tonaldoing/feelolab-releases` (público), verifica que las cuatro versiones coincidan, arma los zips, publica el release allá y deja el tag `vX.Y.Z` acá. Necesita el secret `RELEASES_TOKEN`. También se puede correr a mano desde Actions → Release → Run workflow. Los sitios lo ven en hasta 12 horas, o al instante con "Buscar de nuevo" en Actualizaciones.
